...
namespace PCBuddy_Backend.Services
{
    public class ComputerService
    {
        ...
        // UPDATED: Logic to append Chipset if missing from Name
        public async Task<List<Gpu>> GetGPUs()
        {
            var gpus = await _context.Gpus
                .Where(x => !x.IsDeleted)
                .Select(x => new Gpu { Id = x.Id, Name = x.Name, Chipset = x.Chipset })
                .ToListAsync();

            foreach (var gpu in gpus)
            {
                if (!string.IsNullOrEmpty(gpu.Chipset) &&
                    !gpu.Name.Contains(gpu.Chipset, StringComparison.OrdinalIgnoreCase))
                {
                    gpu.Name = $"{gpu.Name} {gpu.Chipset}";
                }
            }
            return gpus;
        }
        ...
        public async Task<PersonalPC> SavePCConfiguration(int userId, SavePCRequest parts)
        {
            ...
            if (existingPC != null)
            {
                existingPC.CpuId = parts.CpuId; existingPC.GpuId = parts.GpuId; existingPC.MemoryId = parts.MemoryId;
                existingPC.StorageId = parts.StorageId; existingPC.StorageId2 = parts.StorageId2; existingPC.MotherboardId = parts.MotherboardId;
                existingPC.PowerSupplyId = parts.PowerSupplyId; existingPC.CaseId = parts.CaseId; existingPC.TotalPrice = totalPrice;
                existingPC.AddToProfile = parts.AddToProfile; if (parts.Rating.HasValue) existingPC.Rating = parts.Rating;
        ...
        private async Task IncrementUsageCount(SavePCRequest parts)
        {
            ...
            await UpdateCount(_context.Cpus, parts.CpuId); await UpdateCount(_context.Gpus, parts.GpuId); await UpdateCount(_context.Memory, parts.MemoryId);
            await UpdateCount(_context.Storages, parts.StorageId); await UpdateCount(_context.Storages, parts.StorageId2); await UpdateCount(_context.Motherboards, parts.MotherboardId);
            await UpdateCount(_context.PowerSupplies, parts.PowerSupplyId); await UpdateCount(_context.Cases, parts.CaseId);
        }
        ...
        public async Task<PersonalPC> UpdatePCRating(int userId, int pcId, double rating)
        {
            ...
            pc.Rating = rating; pc.UpdatedAt = DateTime.UtcNow;
            await _context.SaveChangesAsync(); return pc;
        }
        ...
        public async Task<SystemPartsResponse> GetPartDetails(SavePCRequest partIds)
        {
            ...
            if (partIds.CpuId.HasValue) { var p = await _context.Cpus.FindAsync(partIds.CpuId); if (p != null) response.Cpu = MapToCpuDetails(p); }
            if (partIds.GpuId.HasValue) { var p = await _context.Gpus.FindAsync(partIds.GpuId); if (p != null) response.Gpu = MapToGpuDetails(p); }
            if (partIds.MemoryId.HasValue) { var p = await _context.Memory.FindAsync(partIds.MemoryId); if (p != null) response.Memory = MapToMemoryDetails(p); }
            if (partIds.StorageId.HasValue) { var p = await _context.Storages.FindAsync(partIds.StorageId); if (p != null) response.Storage = MapToStorageDetails(p); }
            if (partIds.StorageId2.HasValue) { var p = await _context.Storages.FindAsync(partIds.StorageId2); if (p != null) response.Storage2 = MapToStorageDetails(p); }
            if (partIds.MotherboardId.HasValue) { var p = await _context.Motherboards.FindAsync(partIds.MotherboardId); if (p != null) response.Motherboard = MapToMotherboardDetails(p); }
            if (partIds.PowerSupplyId.HasValue) { var p = await _context.PowerSupplies.FindAsync(partIds.PowerSupplyId); if (p != null) response.PowerSupply = MapToPowerSupplyDetails(p); }
            if (partIds.CaseId.HasValue) { var p = await _context.Cases.FindAsync(partIds.CaseId); if (p != null) response.Case = MapToCaseDetails(p); }
            return response;
        }
        ...
        public async Task<SystemPartsResponse> GetUserPc(int userId)
        {
            var pc = await _context.PersonalPCs
                .Include(p => p.Cpu).Include(p => p.Gpu).Include(p => p.Memory).Include(p => p.Storage)
                .Include(p => p.Storage2).Include(p => p.Motherboard).Include(p => p.PowerSupply).Include(p => p.Case)
        ...
        // --- 4. Get Individual Part By ID ---

        public async Task<CpuDetailsDto?> GetCpuById(int id) { var p = await _context.Cpus.FindAsync(id); return (p != null && !p.IsDeleted) ? MapToCpuDetails(p) : null; }
        public async Task<GpuDetailsDto?> GetGpuById(int id) { var p = await _context.Gpus.FindAsync(id); return (p != null && !p.IsDeleted) ? MapToGpuDetails(p) : null; }
        public async Task<MemoryDetailsDto?> GetMemoryById(int id) { var p = await _context.Memory.FindAsync(id); return (p != null && !p.IsDeleted) ? MapToMemoryDetails(p) : null; }
        public async Task<StorageDetailsDto?> GetStorageById(int id) { var p = await _context.Storages.FindAsync(id); return (p != null && !p.IsDeleted) ? MapToStorageDetails(p) : null; }
        public async Task<MotherboardDetailsDto?> GetMotherboardById(int id) { var p = await _context.Motherboards.FindAsync(id); return (p != null && !p.IsDeleted) ? MapToMotherboardDetails(p) : null; }
        public async Task<PowerSupplyDetailsDto?> GetPowerSupplyById(int id) { var p = await _context.PowerSupplies.FindAsync(id); return (p != null && !p.IsDeleted) ? MapToPowerSupplyDetails(p) : null; }
        public async Task<CaseDetailsDto?> GetCaseById(int id) { var p = await _context.Cases.FindAsync(id); return (p != null && !p.IsDeleted) ? MapToCaseDetails(p) : null; }
        ...
        // UPDATED: Logic to append Chipset if missing from Name for Details View
        private GpuDetailsDto MapToGpuDetails(Gpu p)
        {
            string displayName = p.Name;
            if (!string.IsNullOrEmpty(p.Chipset) &&
                !p.Name.Contains(p.Chipset, StringComparison.OrdinalIgnoreCase))
            {
                displayName = $"{p.Name} {p.Chipset}";
            }

            return new GpuDetailsDto(
                p.Id, displayName, (decimal)(p.Price ?? 0), p.Chipset, p.Memory, p.MemoryType,
